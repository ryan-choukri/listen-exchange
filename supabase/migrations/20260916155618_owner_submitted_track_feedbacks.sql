-- Let an authenticated artist read feedback left on their own submitted tracks.
-- Listener identities are deliberately omitted from the returned payload.
create or replace function public.get_owner_submitted_tracks_with_feedbacks()
returns table (
  id uuid,
  track_id text,
  title text,
